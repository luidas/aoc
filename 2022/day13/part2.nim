...
func find_as(input: seq[seq[char]], start: (int, int)): seq[(int, int)] =
  var 
    a = newSeq[(int, int)]()
    input = input
...
func bfs(heights: seq[seq[int]], start: (int, int)): seq[seq[int]] =
  var
    queue = [start].toDeque
...
let
  ...
  a = find_as(input, start_pos)

echo a.mapIt(bfs(heights, it)[end_pos[1]][end_pos[0]]).filterIt(it != -1).min
