...
func adjustItem(item: int, operation: Operation): int =
   let
...
func turn(monkey_id: int, monkeys: seq[Monkey]): seq[Monkey] =
   let
   ...
   if items.len == 0:
      ...
      let
         item = items[0]
         worry_level = adjustItem(item, monkey.operation)
      var target = 0
      if worry_level mod monkey.test == 0:
         target = monkey.if_true
      else:
         target = monkey.if_false
      result = turn(monkey_id, throwItem(monkey_id, target, worry_level, monkeys))

func round(monkeys: seq[Monkey]): seq[Monkey] =
   var monkeys = monkeys
   for monkey_id in 0..monkeys.len-1:
      monkeys = turn(monkey_id, monkeys)
   result = monkeys

func rounds(monkeys: seq[Monkey], num: int): seq[Monkey] =
   if num == 0:
      result = monkeys
   else:
      result = rounds(round(monkeys), num-1)

echo rounds(monkeys, 20).mapIt(it.inspected).sorted(Descending)[0..1].foldl(a*b)
