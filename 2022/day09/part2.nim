...
const
   ...
   starting_grid = Grid(rope: repeat(starting_position, 10),
...
func move_segment(tail: Position, head: Position): Position =
   let
...
func move_tail(tail: seq[Position], head: Position): seq[Position] =
   var current_head = head
   result = tail
   for index, segment in tail:
      let new_pos = move_segment(segment, current_head)
      result[index] = new_pos
      current_head = new_pos
...
func move(grid: Grid, move: Move): Grid =
   if move.distance == 0:
      ...
      let
         rope = grid.rope
         new_head = move_head(rope[0], move.direction)
         new_tail = move_tail(rope[1..9], new_head)
         new_hist = toHashSet(toSeq(grid.tail_history) & @[new_tail[8]])
         new_grid = Grid(rope: @[new_head] & new_tail, tail_history: new_hist)
